...
import { getFollowers, getFollowings, me } from "@/api/user.api";
...
export default function Followings({
    ...
    const [followings, setFollowings] = useState<MiniUserProfile[]>([])
    const infQuery = useInfiniteQuery({
        queryKey: [`followings:${userId}`],
        queryFn: ({ pageParam = 1 }) => getFollowings(userId, pageParam),
        initialPageParam: 1,
                ...
                <View style={styles.container}>
                    <Surface style={styles.innerContainer}>
                        <View style={styles.titleBar}>
                            <Text style={styles.title}>{i18n.t("profile.following")}</Text>
